...
| Employee | peter@employee.dev     | PeterEmployee@2026!   |
...
|---------------------------|----------|--------------------------------------------------|
| `NEXT_PUBLIC_CONVEX_URL`  | Vercel + `.env.local` | Your Convex deployment URL (set automatically by `convex dev`) |
| `NEXT_PUBLIC_APP_URL`     | Vercel + `.env.local` | Full URL of the app (e.g. `https://your-app.vercel.app`) |
| `JWT_PRIVATE_KEY`         | Convex dashboard | Generated by `npx @convex-dev/auth generate-keys` |
| `JWKS`                    | Convex dashboard | Generated by `npx @convex-dev/auth generate-keys` |
| `SITE_URL`                | Convex dashboard | Your app's public URL — set automatically by `generate-keys` |
...
After deploying, seed the production database with categories and test accounts:
```bash
...
3. Add the following environment variables in the Vercel dashboard:
   ...
   | Variable                  | Value                                      |
   |---------------------------|--------------------------------------------|
   | `NEXT_PUBLIC_CONVEX_URL`  | Your production Convex URL (from `npx convex deploy` output) |
   | `NEXT_PUBLIC_APP_URL`     | Your Vercel deployment URL                 |

   > **Note:** `JWT_PRIVATE_KEY`, `JWKS`, and `SITE_URL` are set on the **Convex side** via `npx convex env set` or the Convex dashboard — not in Vercel.
